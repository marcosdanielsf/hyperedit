...
export function useFFmpeg() {
  ...
  const checkLocalServer = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      console.log(
        `Local FFmpeg server: ${available ? "available" : "not available"}`,
      );
      return available;
    } catch {
  ...
  const isDeadAirRemovalCommand = (command: string): boolean => {
    ...
    return (
      ...
      (lowerCmd.includes("silence") &&
        (lowerCmd.includes("remove") || lowerCmd.includes("dead air")))
    );
  };

  // Remove dead air using specialized endpoint
  const removeDeadAirLocal = useCallback(
    async (
      inputFile: File,
      ...
      console.log(
        `[Local FFmpeg] Removing dead air from ${fileSizeMB} MB file`,
      );
      ...
      if (options.silenceThreshold) {
        formData.append(
          "silenceThreshold",
          options.silenceThreshold.toString(),
        );
      }
      if (options.minSilenceDuration) {
        formData.append(
          "minSilenceDuration",
          options.minSilenceDuration.toString(),
        );
      }

      setStatus("Detecting silence and removing dead air...");

      const response = await fetch(`${FFMPEG_URL}/remove-dead-air`, {
        method: "POST",
        body: formData,
      });
      ...
      if (removedDuration && originalDuration) {
        ...
        console.log(
          `[Local FFmpeg] Original: ${originalDuration}s -> New: ${newDuration}s`,
        );
        setStatus(
          `Removed ${parseFloat(removedDuration).toFixed(1)}s of dead air!`,
        );
  ...
  const processVideoLocal = useCallback(
    async (inputFile: File, ffmpegCommand: string): Promise<string> => {
      const fileSizeMB = (inputFile.size / (1024 * 1024)).toFixed(1);
      ...
      if (isDeadAirRemovalCommand(ffmpegCommand)) {
        console.log(
          "[Local FFmpeg] Detected dead air removal - using specialized endpoint",
        );
      ...
      const response = await fetch(`${FFMPEG_URL}/process`, {
        method: "POST",
        body: formData,
      });
      ...
      console.log(
        "[Local FFmpeg] Response headers:",
        Object.fromEntries(response.headers.entries()),
      );

      const blob = await response.blob();
      console.log(
        "[Local FFmpeg] Blob received, size:",
        blob.size,
        "type:",
        blob.type,
      );
  ...
  const generateChapters = useCallback(
    async (
      ...
      try {
        ...
        const response = await fetch(`${FFMPEG_URL}/generate-chapters`, {
          method: "POST",
          body: formData,
        });
        ...
        setStatus("");
        throw new Error(
          `Chapter generation failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      } finally {
        setProcessing(false);
        setProgress(0);
