...
export function useProject() {
  ...
  const [session, setSessionInternal] = useState<SessionInfo | null>(
  ...
  useEffect(() => {
    if (prevActiveTabIdRef.current !== activeTabId) {
      ...
      console.log(
        `  FROM: "${prevActiveTabIdRef.current}" TO: "${activeTabId}"`,
      );
      console.log("=================================================");
  ...
  const setSession = useCallback(
    (
      sessionOrUpdater:
        | SessionInfo
        | null
        | ((prev: SessionInfo | null) => SessionInfo | null),
    ) => {
  ...
  const checkServer = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const response = await fetch(`${FFMPEG_URL}/health`, {
        method: "GET",
  ...
  const uploadAsset = useCallback(
    async (file: File): Promise<Asset> => {
      ...
      try {
        ...
        if (!currentSession) {
          const createResponse = await fetch(`${FFMPEG_URL}/session/create`, {
            method: "POST",
          });
        ...
        const response = await fetch(
          `${FFMPEG_URL}/session/${currentSession.sessionId}/assets`,
          {
            method: "POST",
            body: formData,
          },
        ...
        const asset: Asset = {
          ...
          thumbnailUrl: result.asset.thumbnailUrl
            ? `${FFMPEG_URL}${result.asset.thumbnailUrl}`
            : null,
        };
  ...
  const deleteAsset = useCallback(
    async (assetId: string): Promise<void> => {
      ...
      await fetch(
        `${FFMPEG_URL}/session/${session.sessionId}/assets/${assetId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const getAssetStreamUrl = useCallback(
    (assetId: string): string | null => {
      if (!session) return null;
      return `${FFMPEG_URL}/session/${session.sessionId}/assets/${assetId}/stream`;
    },
    [session],
  );
  ...
  const refreshAssets = useCallback(async (): Promise<Asset[]> => {
    ...
    const response = await fetch(
      `${FFMPEG_URL}/session/${session.sessionId}/assets`,
    );
    if (!response.ok) {
    ...
    const data = await response.json();
    const serverAssets: Asset[] = (data.assets || []).map(
      (a: {
        id: string;
        type: "video" | "image" | "audio";
        filename: string;
        ...
        width: a.width,
        height: a.height,
        thumbnailUrl: a.thumbnailUrl ? `${FFMPEG_URL}${a.thumbnailUrl}` : null,
        // Add cache-busting timestamp to force reload after file changes (e.g., dead air removal)
        streamUrl: `${FFMPEG_URL}/session/${session.sessionId}/assets/${a.id}/stream?v=${Date.now()}`,
  ...
  const addClip = useCallback(
    (
      ...
      if (duration !== undefined) {
        ...
        console.warn(
          `Asset ${assetId} not found in state, using default duration`,
        );
      }
  ...
  const updateClip = useCallback(
    (clipId: string, updates: Partial<TimelineClip>): void => {
      setClips((prev) =>
  ...
  const deleteClip = useCallback(
    (clipId: string, ripple: boolean = false): void => {
      setClips((prev) => {
  ...
  const moveClip = useCallback(
    (clipId: string, newStart: number, newTrackId?: string): void => {
      setClips((prev) =>
        prev.map((c) => {
  ...
  const resizeClip = useCallback(
    (clipId: string, newInPoint: number, newOutPoint: number): void => {
      setClips((prev) =>
        prev.map((c) => {
          if (c.id !== clipId) return c;
  ...
  const splitClip = useCallback(
    (clipId: string, splitTime: number): string | null => {
  ...
  const createTimelineTab = useCallback(
    (name: string, assetId: string, initialClips?: TimelineClip[]): string => {
  ...
  const closeTimelineTab = useCallback((tabId: string): void => {
    ...
    setActiveTabId((currentId) => {
      if (currentId === tabId) {
        console.log(
          "[closeTimelineTab] Active tab is being closed, switching to main",
        );
        return "main";
      }
      return currentId;
  ...
  const updateTabClips = useCallback(
    (tabId: string, clips: TimelineClip[]): void => {
      setTimelineTabs((prev) =>
        prev.map((tab) => (tab.id === tabId ? { ...tab, clips } : tab)),
      );
    },
    [],
  );
  ...
  const updateTabAsset = useCallback(
    (tabId: string, newAssetId: string, newDuration: number): void => {
      console.log("[updateTabAsset] Called with:", {
        tabId,
        newAssetId,
        newDuration,
      });
  ...
  const updateCaptionStyle = useCallback(
    (clipId: string, styleUpdates: Partial<CaptionStyle>): void => {
      setCaptionData((prev) => {
  ...
  const getCaptionData = useCallback(
    (clipId: string): CaptionData | null => {
  ...
  const loadProject = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (assetsResponse.ok) {
        ...
        const serverAssets: Asset[] = (assetsData.assets || []).map(
          (a: {
            id: string;
            type: "video" | "image" | "audio";
            filename: string;
  ...
  const renderProject = useCallback(
    async (preview = false): Promise<string> => {
      ...
      try {
        ...
        const response = await fetch(
          `${FFMPEG_URL}/session/${session.sessionId}/render`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        // Return download URL
        return `${FFMPEG_URL}${result.downloadUrl}`;
      } finally {
        setLoading(false);
  ...
  const createGif = useCallback(
    async (
      ...
      try {
        const response = await fetch(
          `${FFMPEG_URL}/session/${session.sessionId}/create-gif`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const closeSession = useCallback(async (): Promise<void> => {
    if (session) {
      try {
        await fetch(`${FFMPEG_URL}/session/${session.sessionId}`, {
          method: "DELETE",
        });
      } catch {}
    }
