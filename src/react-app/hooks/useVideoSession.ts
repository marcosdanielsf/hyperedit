...
export function useVideoSession() {
  ...
  const uploadVideo = useCallback(
    async (file: File): Promise<SessionInfo> => {
      ...
      setProcessing(true);
      setStatus(
        `Uploading video (${file.size > 1024 * 1024 * 1024 ? fileSizeGB + " GB" : fileSizeMB + " MB"})...`,
      );
      ...
      try {
        ...
        const response = await fetch(`${FFMPEG_URL}/session/upload`, {
          method: "POST",
          body: formData,
  ...
  const getStreamUrl = useCallback((): string | null => {
    ...
    return `${FFMPEG_URL}/session/${session.sessionId}/stream`;
  }, [session]);

  // Process video (edit in place - no re-upload!)
  const processVideo = useCallback(
    async (command: string): Promise<SessionInfo> => {
      if (!session) {
      ...
      try {
        const response = await fetch(
          `${FFMPEG_URL}/session/${session.sessionId}/process`,
          {
            method: "POST",
  ...
  const removeDeadAir = useCallback(
    async (options?: {
      ...
      try {
        ...
        if (result.removedDuration > 0) {
          setStatus(
            `Removed ${result.removedDuration.toFixed(1)}s of dead air!`,
          );
        } else {
  ...
  const downloadVideo = useCallback(async (): Promise<void> => {
    ...
    link.href = `${FFMPEG_URL}/session/${session.sessionId}/download`;
  ...
  const refreshSession = useCallback(async (): Promise<SessionInfo | null> => {
    ...
    try {
      const response = await fetch(
        `${FFMPEG_URL}/session/${session.sessionId}/info`,
      );
      if (!response.ok) {
        setSession(null);
        return null;
